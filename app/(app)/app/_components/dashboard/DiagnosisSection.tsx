...
export function DiagnosisSection({ diagnosis, loading = false, presentation = "default" }: DiagnosisSectionProps) {
  const isHero = presentation === "hero";

  return (
    <section className={isHero ? "h-full" : "space-y-3"} data-testid="dashboard-diagnosis-section">
      {isHero ? null : (
      ...
      <PanelCard
        className={`h-full border-brand-border/75 ${
          isHero
            ? "bg-[linear-gradient(155deg,rgba(16,32,67,0.96),rgba(22,48,97,0.92),rgba(16,32,67,0.96))]"
      ...
      >
        {loading && !diagnosis.hasTypedDiagnosis ? (
          ...
          <div className={isHero ? "space-y-5" : "space-y-4"}>
            ...
            <div className={diagnosis.comparisonContext && !isHero ? "grid gap-4 lg:grid-cols-[minmax(0,1.65fr)_minmax(0,1fr)]" : "space-y-4"}>
              <article
                className={`rounded-[1.2rem] border border-brand-border-strong/70 p-5 shadow-brand-card ${
                  isHero
                    ? "bg-[linear-gradient(155deg,rgba(20,43,86,0.9),rgba(14,30,60,0.92))]"
                    : "bg-[linear-gradient(155deg,rgba(19,41,80,0.82),rgba(16,32,67,0.9))]"
                }`}
              >
                <div className="flex flex-wrap items-start justify-between gap-3">
                  <div>
                    <p className="text-[11px] uppercase tracking-[0.14em] text-brand-text-secondary">
                      {isHero ? "Biggest constraint" : "Latest report diagnosis"}
                    </p>
                    <h3 className={`${isHero ? "mt-3 text-2xl" : "mt-2 text-xl"} font-semibold leading-snug text-brand-text-primary`}>
                      {diagnosis.heading}
                    </h3>
                  </div>
                  {diagnosis.hasTypedDiagnosis ? (
                    <span className="rounded-full border border-brand-border/75 bg-brand-panel/75 px-3 py-1 text-[11px] uppercase tracking-[0.12em] text-brand-text-muted">
                      From current report evidence
                    </span>
                  ) : null}
                </div>

                {diagnosis.summary ? (
                  <p className={`${isHero ? "mt-4" : "mt-3"} text-sm leading-relaxed text-brand-text-secondary`} data-testid="dashboard-diagnosis-summary">
                    {diagnosis.summary}
                  </p>
                ) : (
                  <p className={`${isHero ? "mt-4" : "mt-3"} text-sm leading-relaxed text-brand-text-secondary`} data-testid="dashboard-diagnosis-unavailable">
              ...
              {diagnosis.comparisonContext ? (
                <article
                  className={`rounded-[1.2rem] border border-brand-border-strong/70 p-5 shadow-brand-card ${
                    isHero
                      ? "bg-[linear-gradient(155deg,rgba(17,38,73,0.92),rgba(13,28,55,0.94))]"
                      : "bg-[linear-gradient(155deg,rgba(17,38,73,0.88),rgba(16,32,67,0.92))]"
                  }`}
            ...
            {diagnosis.supportingMetrics.length > 0 ? (
              <ul className={`grid grid-cols-1 gap-3 ${isHero ? "" : "md:grid-cols-3"}`} data-testid="dashboard-diagnosis-supporting-metrics">
