...
function implicationFor(categoryId: InsightCategoryId, variant: DashboardInsightVariant): string {
  if (categoryId === "platform-risk") {
    if (variant === "warning") {
  ...
  if (categoryId === "creator-stability") {
    if (variant === "warning") {
...
function buildCard(candidate: InsightCandidate, cardIndex: number): DashboardInsightCard {
  const implication = clampCopy(implicationFor(candidate.category.id, candidate.variant), MAX_IMPLICATION_LENGTH);

  return {
    id: `insight-${candidate.category.id}-${cardIndex + 1}`,
...
export function buildDashboardInsights(input: BuildDashboardInsightsInput): DashboardInsightCard[] {
  ...
  const keySignals = uniqueNonEmpty(input.keySignals);
  if (keySignals.length === 0) {
    return [];
  }

  const candidates = keySignals.map<InsightCandidate>((signal, sourceIndex) => ({
    signal,
