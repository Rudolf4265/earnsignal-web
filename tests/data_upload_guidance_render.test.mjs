...
test("data upload page consumes canonical source manifest and workspace readiness truth", async () => {
  ...
  assert.equal(source.includes("This report will use your staged workspace sources."), true);
  assert.equal(source.includes("Contribution"), true);
  assert.equal(source.includes("Next run"), true);
  assert.equal(source.includes("card.guidance"), false);
  assert.equal(source.includes("card.roleSummary"), false);
  assert.equal(source.includes("card.knownLimitations"), false);
