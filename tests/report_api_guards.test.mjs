...
test("createReportRun omits selected_platforms from request body when platform list is empty", async () => {
  ...
  global.fetch = async (url, init = {}) => {
    ...
    return {
      ...
      status: 200,
      headers: { get: (key) => (key === "content-type" ? "application/json" : null) },
      text: async () => JSON.stringify({ report_id: "rep_run_empty_456" }),
    };
  };
  ...
  try {
    ...
    assert.deepEqual(requests, [{ url: "https://api.example.test/v1/reports/run", method: "POST", body: null }]);
    assert.equal(result.reportId, "rep_run_empty_456");
  } finally {
    global.fetch = originalFetch;
  }
});
...
test("createReportRun sends explicit selected_platforms in submit body when platforms are non-empty", async () => {
  ...
  global.fetch = async (url, init = {}) => {
    ...
    return {
      ...
      status: 200,
      headers: { get: (key) => (key === "content-type" ? "application/json" : null) },
      text: async () => JSON.stringify({ report_id: "rep_run_patreon_789" }),
    };
  };

  try {
    const { createReportRun } = await loadReportsModule(Date.now() + 7);
    const result = await createReportRun({ selectedPlatforms: ["patreon"] });
    ...
    assert.equal(result.reportId, "rep_run_patreon_789");
  } finally {
    global.fetch = originalFetch;
  }
});
...
test("createReportRun propagates backend rejection when platform selection is invalid", async () => {
  ...
  global.fetch = async () => {
    return {
      ...
      status: 422,
      headers: { get: (key) => (key === "content-type" ? "application/json" : null) },
      text: async () => JSON.stringify({ message: "INVALID_PLATFORM_SELECTION", code: "INVALID_PLATFORM_SELECTION" }),
    };
  };

  try {
    const { createReportRun } = await loadReportsModule(Date.now() + 8);
    await assert.rejects(() => createReportRun({ selectedPlatforms: ["invalid_platform"] }), /INVALID_PLATFORM_SELECTION/);
  } finally {
    global.fetch = originalFetch;
  }
});
